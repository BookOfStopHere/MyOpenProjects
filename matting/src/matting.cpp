...
double BayesianMatting::Solve()
{
	...
	for(int Iteration = 0 ; Iteration<1; ++Iteration )
	{
		...
		{				
			...
			for(int k = 0; k < (int)toSolveList.size(); ++k)
			...
			for( p = 0 ; p < (int)toSolveList.size() ;++p )
			{	
...
#define WIN_SIZE 1
	...
	min_x = std::max(0, c - WIN_SIZE);
	min_y = std::max(0, r - WIN_SIZE);
	max_x = std::min(colorImg->width - 1, c + WIN_SIZE );
	max_y = std::min(colorImg->height - 1, r + WIN_SIZE );
...
#define UNSURE_DISTANCE 1	
	...
	{		
		if( r - dist >= 0 )
		{
			for(int z = std::max(0, c - dist); z<= std::min(colorImg->width-1, c+dist); ++z )
			{ 
		...
		if( c - dist >= 0)
		{			
			for(int z = std::max(0, r - dist+ 1); z<= std::min(colorImg->height-1, r+dist - 1 ); ++z )
			{
...
void BayesianMatting::SolveBF( int r, int c, CvMat *fg_mean, CvMat *inv_fg_cov, CvMat *bg_mean, CvMat *inv_bg_cov )
{
	...
	CV_IMAGE_ELEM( fgImg, uchar, r, 3*c )   = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 0, 0 )));
	CV_IMAGE_ELEM( fgImg, uchar, r, 3*c+1 ) = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 1, 0 )));
	CV_IMAGE_ELEM( fgImg, uchar, r, 3*c+2 ) = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 2, 0 )));
	CV_IMAGE_ELEM( bgImg, uchar, r, 3*c )   = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 3, 0 )));
	CV_IMAGE_ELEM( bgImg, uchar, r, 3*c+1 ) = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 4, 0 )));
	CV_IMAGE_ELEM( bgImg, uchar, r, 3*c+2 ) = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 5, 0 )));
...
inline void BayesianMatting::SolveAlpha(int r, int c)
{
	...
	CV_IMAGE_ELEM( alphamap, float, r, c ) = std::max( (float)0, std::min( (float)1, CV_IMAGE_ELEM( alphamap, float, r, c )));
}
...
double RobustMatting::Solve()
{
	...
	{		
		...
		for(i=0;i<length;++i)
		{
			x[i] = std::min( (double)1, std::max( (double)0, x[i]));
		}
	}
	...
	for(i=0;i<length;++i)
	{
		 CV_IMAGE_ELEM( alphamap, float, unknown_list[i].y, unknown_list[i].x ) = std::min( 1.f, std::max(0.f, (float)x[i]));
	}
...
float RobustMatting::SmoothWeight( int r1, int c1, int r2, int c2, CvMat* mat_3x3 )
{
	...
	int start_r = std::max( 0, max_r - 2 ), start_c = std::max( 0, max_c - 2 );
	int end_r = std::min( colorImg->height-3, min_r ), end_c = std::min( colorImg->width-3, min_c );
...
void FlashMatting::GenerateTrimap( IplImage *FlashOnlyImg, IplImage *&Trimap, float level, float ratio )
{
    ...
    for(int i=0;i<128;++i)
    {
            ...
            for(int j=std::max(0,i-10);j<=std::min(127,i+10);++j)
            {
#define VARIANCE 7.f
	...
	{	
		...
		for(int i=0;i<Trimap->height;++i)
			for(int j=0;j<Trimap->width;++j)	
				for(int di = std::max(0,i-1);di<=std::min(Trimap->height-1,i+1);++di)
					for(int dj = std::max(0,j-1);dj<=std::min(Trimap->width-1,j+1);++dj)
						CV_IMAGE_ELEM( dilationImg, uchar, i, j ) = std::max( CV_IMAGE_ELEM( dilationImg, uchar, i, j ) , CV_IMAGE_ELEM( tmp, uchar, di, dj ) );
...
double FlashMatting::Solve()
{
	...
	vector<CvMat *> fg_mean( FLASH_MAX_CLUS, NULL );
	vector<CvMat *> bg_mean( FLASH_MAX_CLUS, NULL );
	vector<CvMat *> flash_fg_mean( FLASH_MAX_CLUS, NULL );
	vector<CvMat *> inv_fg_cov( FLASH_MAX_CLUS, NULL );
	vector<CvMat *> inv_bg_cov( FLASH_MAX_CLUS, NULL );
	vector<CvMat *> inv_flash_fg_cov( FLASH_MAX_CLUS, NULL );
...
#define UNSURE_DISTANCE 1	
	...
	{		
		if( r - dist >= 0 )
		{
			for(int z = std::max(0, c - dist); z<= std::min(ambientImg->width-1, c+dist); ++z )
			{ 
				dist_weight = expf( -(dist*dist+(z-c)*(z-c)) * inv_2sigma_square );
		...
		if( c - dist >= 0)
		{			
			for(int z = std::max(0, r - dist+ 1); z<= std::min(ambientImg->height-1, r+dist - 1 ); ++z )
			{
				dist_weight = expf( -((z-r)*(z-r)+dist*dist) * inv_2sigma_square );
...
#define WIN_SIZE 1
	...
	min_x = std::max(0, c - WIN_SIZE);
	min_y = std::max(0, r - WIN_SIZE);
	max_x = std::min(ambientImg->width - 1, c + WIN_SIZE );
	max_y = std::min(ambientImg->height - 1, r + WIN_SIZE );
...
void FlashMatting::SolveBFF_( int r, int c, CvMat *fg_mean, CvMat *inv_fg_cov, CvMat *bg_mean, CvMat *inv_bg_cov, CvMat *flash_fg_mean, CvMat *inv_flash_fg_cov )
{
	...
	CV_IMAGE_ELEM( bgImg, uchar, r, 3*c+1 )      = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 4, 0 )));
	CV_IMAGE_ELEM( bgImg, uchar, r, 3*c+2 )      = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 5, 0 )));
	CV_IMAGE_ELEM( flashFgImg, uchar, r, 3*c )   = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 6, 0 )));
	CV_IMAGE_ELEM( flashFgImg, uchar, r, 3*c+1 ) = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 7, 0 )));
	CV_IMAGE_ELEM( flashFgImg, uchar, r, 3*c+2 ) = (uchar)std::max( (float)0, std::min( (float)255, CV_MAT_ELEM( *x, float, 8, 0 )));
...
void PoissonMatting::ReconstructAlpha()
{
	...
	for (r = 1; r < unmask->height-1; r++) {
		for (c = 1; c < unmask->width-1; c++) {
			...
			{
				int id = r*unmask->width+c;				
				CV_IMAGE_ELEM( alphamap, float, r, c ) = std::max( 0.f, std::min( 1.f, solValues[mp[id]] ));
			}						
		}
	}
...
int PoissonMatting::nearestPoint(int r, int c, int &nr, int &nc, IplImage *mask)
{
	...
	{
		if(r-dist>=0)
			for(j=std::max(0,c-dist);j<=std::min(mask->width-1,c+dist);++j )
		...
		if(c-dist>=0)
			for(i=std::max(0,r-dist);i<=std::min(mask->height-1,r+dist);++i )
